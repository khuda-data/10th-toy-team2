"""최종 ETA 통합 엔진 (역할③: 평가·검증·통합 — 권동하).

역할①(k_slope, v_user)과 역할②(신호대기) 산출물을 조립해
"경로 + 사람 -> 예상 소요시간(초)"를 계산한다.

인터페이스 계약 (팀원 산출물이 갱신되면 파일만 교체하면 됨):
- 역할①: data/processed/k_slope_model.json  {"coef": [b2, b1, b0]}  (np.polyfit 순서)
          data/processed/v_user.csv         person, v_user_mps 컬럼 필수
- 역할②: data/processed/signal_model.json   {"alpha": float, "beta": float}
...
class EtaEngine:
    ...
    def load(cls, root: str | Path = ".") -> "EtaEngine":
        """역할① v2 산출물(models/)을 우선 사용, 없으면 v1(data/processed/) fallback.

        v2 (최희수, 2026-08-20): 1차식 + valid_slope_range 클립 + 경사보정 v_user.
        """
        root = Path(root)
        ...
        # 역할②(신호대기) 산출물: 횡단보도별 기대대기가 이미 계산되어 있음
    ...
    def expected_wait_s(self, cycle_C_s: float, red_R_s: float) -> float:
        """이론공식 E(Wait)=R^2/(2C)에 역할② 보정회귀(alpha+beta*이론값)를 적용."""
    ...
    def route_signal_wait_s(self, segments: pd.DataFrame, radius_m: float = 25.0,
        ...
        역할② 산출물(`영통구_횡단보도별_신호대기.csv`)은 지점별 기대대기(`wait_s`)를
